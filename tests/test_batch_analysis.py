...
class BatchRuntimeTests(unittest.TestCase):
    ...
    def test_batch_returns_report_and_chart_despite_review_and_partial_failure(self):
        ...
        self.assertIn("✅ 已收到产品反馈", self.feishu.messages[0][1])
        self.service.handle_command({"chat_id": "chat1", "text": "/报告"})
        self.assertIn("好评原因", self.feishu.messages[-1][1])
        self.assertEqual(len([item for item in self.feishu.images if item[0] == "chat1"]), 4)
