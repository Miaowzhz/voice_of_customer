...
│   │   └── feishu.py               # 飞书 API 封装
│   ├── repositories/
│   │   ├── sqlite.py               # 运行、反馈和 Issue 持久化
│   │   └── bitable.py              # 多维表读写
│   └── prompts/
│       ├── classify.md             # 分类 Prompt
│       └── issue.md                # Issue 建议 Prompt
├── tests/
├── scripts/
│   ├── seed_mock.py                # 导入 mock 数据
...
2. 配置事件订阅地址 `/webhooks/feishu`，处理 URL 验证和消息/文件事件。
3. FastAPI 验证请求来源和签名，使用 `event_id` 去重。
4. 解析文本、附件和发送人上下文，写入本地任务表。
5. 立即回复受理状态，后台启动 LangGraph。

文件下载、表格写入和消息发送全部封装在 `FeishuClient` 中，统一处理访问令牌刷新、超时、限流和错误码。
