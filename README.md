...
3. 机器人收到数据后返回条数与运行编号，完成后发送分析摘要、等级饼图和好评词云。摘要包含产品、好中差等级数量与占比、好评原因、中评改进、差评原因、典型反馈和建议动作，随后发送等级饼图和好评词云。图片上传需要应用具备相应图片资源权限。[上传图片接口](https://open.feishu.cn/document/server-docs/im-v1/image/create)
